...
LinearSVC có spam F1 cao hơn Logistic Regression và giảm số spam bị bỏ sót từ 9 xuống 4. Tuy nhiên, số false positive cũng tăng từ 9 lên 12, nghĩa là có thêm trường hợp ham bị dự đoán nhầm thành spam. Vì vậy, khi đánh giá model cần xem xét đồng thời false positive và false negative thay vì chỉ ưu tiên recall.
...
Trade-off này cần được cân nhắc theo mục tiêu triển khai. Nếu ưu tiên bắt spam mạnh hơn, threshold âm giúp giảm false negative. Nếu ưu tiên hạn chế đưa nhầm email hợp lệ vào spam, threshold mặc định hoặc threshold bảo thủ hơn sẽ an toàn hơn vì giúp giảm false positive.
...
Tức là SVM bắt thêm được 5 spam bị Logistic Regression bỏ sót, nhưng đánh nhầm thêm 3 ham thành spam. Đây là trade-off rõ ràng: model nhạy hơn với spam nhưng cũng làm tăng rủi ro phân loại nhầm nội dung hợp lệ thành spam.
...
Nếu mục tiêu là cân bằng giữa hiệu năng và hạn chế false positive, nên ưu tiên:
...
ngram_range = [1, 2]
min_df = 2
threshold = 0.0, tức default decision boundary
```

Lý do:

LinearSVC default decision boundary có spam F1 cao: 0.9705.
False positive thấp hơn bản tuned threshold: 5 so với 12.
False negative vẫn ở mức chấp nhận được: 8.
Cấu hình này an toàn hơn nếu muốn giảm rủi ro ham bị dự đoán nhầm thành spam.
Bản tuned threshold -0.2165 bắt được nhiều spam hơn, nhưng đổi lại false positive tăng rõ rệt.
